...
void HostWindow::SetContentSize(const WindowSizeRequest& size) {
  if (size.has_preferred_view_size) {
    ...
    if (window_size) {
      auto const constrained_size =
          box_constraints_.constrain(window_size.value());
      SetWindowPos(window_handle_, NULL, 0, 0, constrained_size.width(),
...
void HostWindow::SetConstraints(const WindowConstraints& constraints) {
  ...
  if (constraints.has_view_constraints) {
    smallest = Size(constraints.view_min_width, constraints.view_min_height);
    if (constraints.view_max_width > 0 && constraints.view_max_height > 0) {
      biggest = Size(constraints.view_max_width, constraints.view_max_height);
    }
  }
  ...
  const Size constrained_size = box_constraints_.constrain(current_size);
  if (current_size != constrained_size) {
    SetWindowPos(window_handle_, NULL, 0, 0, constrained_size.width(),
                 constrained_size.height(),
                 SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
  }
...
void HostWindow::SetFullscreen(
  ...
  if (fullscreen) {
    ...
    HMONITOR monitor =
        MonitorFromWindow(window_handle_, MONITOR_DEFAULTTONEAREST);
    if (display_id) {
      for (auto const& display : engine_->display_manager()->displays()) {
